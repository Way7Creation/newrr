...
class CompleteIndexer {
        ...
        try {
            $stmt = $this->pdo->prepare("
                SELECT p.*, 
                       ...
                       COALESCE(pr.base_price, 0) AS base_price,
                       COALESCE(pr.has_special, 0) AS has_special,
                       COALESCE(sb.quantity, 0) AS stock_quantity,
                       COALESCE(sb.available, 0) AS stock_available,
                       GROUP_CONCAT(pi.url SEPARATOR ',') AS image_urls
                FROM products p
    ...
    private function enrichProductData(array &$product): void {
        ...
        // Обработка цен
        $product['price'] = [
            'final' => (float)($product['price'] ?? 0),
            'base' => (float)($product['base_price'] ?? 0),
            'has_special' => (bool)($product['has_special'] ?? false)
        ];
        
        // Обработка наличия
        $product['stock'] = [
            'quantity' => (int)($product['stock_quantity'] ?? 0),
            'available' => (bool)($product['stock_available'] ?? false)
        ];
        
        // Обработка доступности
        $product['availability'] = [
            'text' => $this->getAvailabilityText($product)
        ];
        
        // Обработка изображений
        $product['images'] = $this->getProductImages($product);
        $product['image_url'] = $this->getProductImageUrl($product);
        $product['image_urls'] = $this->getProductImageUrls($product);
        
        // Дефолтные значения для дополнительных полей
        $product['_exact_match'] = false;
        $product['_formatted_name'] = $product['name'];
        $product['_highlight'] = [];
